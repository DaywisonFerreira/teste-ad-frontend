...
import { Container, Content, ListParticipants, UserActions, Loading } from './styles';
...
function Users() {
  ...
  async function handleSorted() {
    ...
    try {
      ...
      toast.error('Erro ao realizar o sorteio, tente novamente mais tarde')
    }
    setLoading(false)
  }

  async function handleDelete(userId) {
    const confirm = window.confirm('Tem certeza que deseja excluir este participante?')
    if (confirm) {
      try {
        ...
        toast.error('Erro ao excluir o participante, tente novamente mais tarde')
      }
    }
  }
  ...
  return (
    <Container>
      <Content>
        ...
        <p>Obs: Para realizar o sorteio deve ter pelo menos 3 participantes cadastrados.</p>
        {users.length === 0 &&
            ...
            {users.map(user => (
              <ListGroup.Item variant="light" className="list">
                ...
                <UserActions>
                  ...
                  <MdModeEdit className="icon-edit" title="Editar"/>
                  </Link>
                  <button onClick={() => handleDelete(user._id)}>
                    <MdDelete className="icon-delete" title="Deletar" />
                  </button>
                  {user.friend && (
                    <button onClick={() => handleShowFriend(user.friend)}>
